...
class Configurator: NSObject {
    ...
    class func configuredCharactersAsyncLoadConfiguration() -> AsyncLoadConfiguration {
        return AsyncLoadConfiguration(responseParsingBlock: { result in
            ...
            for item: [AnyHashable : Any]? in items as? [[AnyHashable : Any]?] ?? [[AnyHashable : Any]?]() {
                autoreleasepool {
                    let article = Article(identifier: item?[kId] as? String ?? "", title: item?[kTitle] as? String ?? "", abstract: item?[kAbstract] as? String ?? "", urlString: "\(basePath ?? "")\(item?[kArticleURL])", thumbnailURLString: item?[kThumbnailURL] as? String ?? "")

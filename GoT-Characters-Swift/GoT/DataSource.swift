...
class DataSource: NSObject, UITableViewDataSource {
    ...
    func addItems(_ items: [Any]!) {
        ...
        mutableItems = (mutableItems as NSArray).sortedArray(using: [NSSortDescriptor(key: "title", ascending: true)]) as? [AnyHashable] ?? mutableItems
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if item != nil {
            cellConfigureBlock?(cell ?? UITableViewCell(), indexPath, item ?? 0)
        }
        if let aCell = cell {
            return aCell
        }
